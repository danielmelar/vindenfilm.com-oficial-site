...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  if (!accessKey) {
    toast.error("A chave de acesso está faltando. Por favor, configure a variável de ambiente.");
    return;
  }
...
const handleWhatsApp = () => {
  ...
  const phoneNumber = "5511963171341";
      ...
      >
        ...
        <div className="flex flex-col justify-center gap-6 md:gap-8">
          <div className="flex flex-col gap-4 md:gap-5">
            ...
            <a href="tel:+5511963171341" className="flex items-center gap-3 text-foreground hover:text-primary transition-colors font-body text-sm">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 md:gap-5 h-full justify-center">
          ...
          <div className="flex flex-col gap-3 mt-2">
            <button
              ...
              className="w-full bg-primary text-primary-foreground font-body text-sm uppercase tracking-[0.2em] px-8 py-3 hover:bg-primary/80 transition-colors"
              disabled={isSubmitting}
            >
              Enviar Mensagem
            </button>
            <button
              type="submit"
              className="w-full bg-transparent border border-primary text-primary font-body text-sm uppercase tracking-[0.2em] px-8 py-3 hover:bg-primary hover:text-primary-foreground transition-colors"
              disabled={isSubmitting}
            >
